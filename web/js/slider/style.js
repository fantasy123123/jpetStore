(function (window, document) {
    ...
    SliderBar.prototype = {
        ...
        reToNewImg: function () {
            ...
            var imgSrc = "images/slide/" + _this.imgList[index] + "";
        ...
        cutImg: function () {
            ...
            // left
            _this.resultX = Math.floor(Math.random() * (_this.imgWidth - cutWidth * 2) + cutWidth);
            // top
                ...
                if(_this.resultX > (left - 2) && _this.resultX < (left + 2)){
                    ...
                    function login(){
                        if(xhr.readyState == 4 && xhr.status == 200){
                                ...
                                //判断是否登录成功
                                if(array[0] == 'false'){
                                    window.alert("Invalid username or password. Sign in failed.");
                                    ...
                                    if(array[0] == "true"){
                                        ...
                                        //现有购物车不为空，提示用户选择是否添加到自己的购物车
                                        if(array[0] == "UpdateCartOrNot"){
                                            var xhrTemp = new XMLHttpRequest();
                                            if(confirm("Login successfully! \nDo you want to add the current cart to your previous cart?")){
                                                xhrTemp.open("POST","loadCart");
                                                xhrTemp.send(null);
                                                window.location.href = info;
                                            }
                                        }
